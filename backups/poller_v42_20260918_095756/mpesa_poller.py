...
def _finalize_with_retry(sale_id: int, max_attempts: int = 3):
    """
    Call _finalize_paid_sale with retries on SQLite lock errors.

    Why: SQLite can reject a write with "database is locked" if another
    process holds the write lock at that instant. With WAL mode enabled
    in database.py, this is rare, but we still retry to be safe — losing
    stock/tax data is worse than waiting 2 seconds.
    """
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if "locked" in msg or "busy" in msg:
                ...
                # Not a locking error - no point retrying
                raise
