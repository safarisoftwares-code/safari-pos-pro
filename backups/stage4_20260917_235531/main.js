...
async function loadSuppliers() {
    try {
        ...
        if (!suppliers || suppliers.length === 0) {
            tbody.innerHTML = "<tr><td colspan=\"8\" style=\"text-align:center;color:#999;padding:20px\">No suppliers yet. Click \"Add Supplier\" to create one.</td></tr>";
            return;
        }
        ...
        tbody.innerHTML = suppliers.map(s =>
            ...
            "<td>" +
                "<button onclick=\"openEditSupplierModal(" + s.id + ")\" style=\"padding:5px 10px;font-size:10px;margin-right:3px;background:#2e7d32;color:white;border:none;border-radius:3px;cursor:pointer\">Edit</button>" +
                "<button onclick=\"deleteSupplier(" + s.id + ")\" style=\"padding:5px 10px;font-size:10px;color:red;cursor:pointer\">Delete</button>" +
            "</td>" +
            "</tr>"
        ).join("");
    } catch (err) {
...
async function openEditSupplierModal(supplierId) {
    try {
        ...
        document.getElementById("supplierKraPin").value = s.kra_pin || "";
...
async function saveSupplier(event) {
    ...
    const payload = {
        ...
        kra_pin: document.getElementById("supplierKraPin").value.trim() || null,
        notes: document.getElementById("supplierNotes").value.trim() || null,
    };

    try {
        if (id) {
            await apiCall("/suppliers/" + id, "PUT", payload);
...
async function deleteSupplier(supplierId) {
    if (!confirm("Deactivate this supplier? Existing POs will keep the name.")) return;
    try {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (supplierForm) {
        supplierForm.addEventListener("submit", singleSubmit(saveSupplier));
    }
